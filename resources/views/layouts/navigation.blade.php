<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    ...
                    @auth
                        @if(auth()->user()->isClient())
                            <x-nav-link :href="route('travaux.index')" :active="request()->routeIs('travaux.*')">
                                {{ __('Mes Travaux') }}
                            </x-nav-link>
                        @elseif(auth()->user()->isFreelance())
                            <x-nav-link :href="route('travaux.all')" :active="request()->routeIs('travaux.all')">
                                {{ __('Travaux disponibles') }}
                            </x-nav-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @auth
                @if(auth()->user()->isClient())
                    <x-responsive-nav-link :href="route('travaux.index')" :active="request()->routeIs('travaux.*')">
                        {{ __('Mes Travaux') }}
                    </x-responsive-nav-link>
                @elseif(auth()->user()->isFreelance())
                    <x-responsive-nav-link :href="route('travaux.all')" :active="request()->routeIs('travaux.all')">
                        {{ __('Travaux disponibles') }}
                    </x-responsive-nav-link>
